...
double exactSol(const Vector &x, double time);
...
void getVort(int dim, const Vector &aux_grad, Vector &vort);
void getEntropyVariables(int dim, const Vector &u_sol, Vector &entropy_var);
void dot(int dim, const Vector &entropy_var, const Vector &resi, Vector &entr_dot_resi);
void ComputeMaxError(FiniteElementSpace &fes, GridFunction &uD, int vDim, Vector &maxError);
void ComputeError(int dim, const Vector &u_sol, const Vector &u_exact, Vector &error);
...
private:
    ...
    GridFunction exact_solution;   
    GridFunction error;   

    GridFunction rhs;   
    GridFunction vort;   

    GridFunction entropy_var;   
    GridFunction entr_dot_resi;   
...
CNS::CNS() 
{
   ...
   {
       if (adapt)
           eleOrder[i] = order + 2; // Initializing with uniform order + 2 when doing adaptation
       else
   ...
   rhs.SetSpace(fes);
   entropy_var.SetSpace(fes);
   vort.SetSpace(fes_op);
   entr_dot_resi.SetSpace(fes_op);
   exact_solution.SetSpace(fes_op);
   error.SetSpace(fes_op);

   FunctionCoefficient ex(exactSol);
   ex.SetTime(0.0);
   ...
   getEntropyVariables(dim, u_sol, entropy_var);
   dot(dim, entropy_var, rhs, entr_dot_resi);

   adv->Mult(u_sol, rhs);

   getAuxGrad(dim, k_vis_x->SpMat(), k_vis_y->SpMat(), adv->GetMSolver(), u_sol, 
                  b_aux_x, b_aux_y, aux_grad);
   postProcess(*mesh, *vfec, u_sol, aux_grad, ele_order_pp, error, entr_dot_resi, rhs, ti, t);

   adv->SetTime(t);

   Vector maxError(fes->GetNE()); // Max error in each element
...
//          ComputeMaxError(*fes, entr_dot_resi, 0, maxError);
          ComputeMaxError(*fes, rhs, 0, maxError);

          UpdateElementOrder(maxError, tolerance, newEleOrder);
...
void CNS::Update(Array<int> &newEleOrder)
{
   ...
   if (isUpdate)
   {
       ...
       vort.Update();
       entropy_var.Update();
       entr_dot_resi.Update();
       exact_solution.Update();
       error.Update();
...
void CNS::UpdateElementOrder(Vector &error, double tolerance, Array<int> &newEleOrder)
{
   ...
   for (int i = 0; i < ne; i++) 
   {
       ...
       if (abs(error[i]) > 100*tolerance)
       {
           newEleOrder[i] = order + 3; 
       }
       else if (abs(error[i]) > 10*tolerance)
       {
...
void getVort(int dim, const Vector &aux_grad, Vector &vort)
{
    ...
    for (int i = 0; i < dofs; i++)
    {
        ...
        if (dim == 2)
        {
           vort[i] = abs(u_grad[1][0] - u_grad[0][1]);
        }
        else if (dim == 3)
        {
...
void getEntropyVariables(int dim, const Vector &u_sol, Vector &entropy_var)
{
    ...
    for (int i = 0; i < dofs; i++)
    {
        ...
        double p     = (E - 0.5*rho*v_sq)*(gamm - 1);

        double S     = log(p) - gamm*log(rho);
        
        entropy_var[                  i] = ((gamm - S)/(gamm - 1)) - (rho*v_sq)/(2*p);
        
        for (int j = 0; j < dim; j++)
        {
            entropy_var[(1 + j)*dofs + i] = rho*vel[j]/p;
        }

        entropy_var[(vDim - 1)*dofs + i] = -rho/p;
    }
}
...
{
    int vDim    = dim + 2; 
    int dofs    = entropy_var.Size()/vDim;
    ...
    for (int i = 0; i < dofs; i++)
    {
        ...
        {
            entr_dot_resi[i] += resi[j*dofs + i] * entropy_var[j*dofs + i];
        }
        entr_dot_resi[i] =  abs(entr_dot_resi[i]);
    }
}
...
void ComputeError(int dim, const Vector &u_sol, const Vector &u_exact, Vector &error)
{
    ...
    for (int i = 0; i < dofs; i++)
    {
        ...
        if (problem == 0)
        {
            error[i]     = abs(rho - u_exact[i]);
        }
        else if (problem == 1)
        {
            error[i]     = abs(p - u_exact[i]);
        }
    }
}
...
// Returns the max Error of the componenent of the GridFunction specified by vDim
void ComputeMaxError(FiniteElementSpace &fes, GridFunction &uD, int vDim, Vector &maxError)
{
   ...
   for (int i = 0; i < fes.GetNE(); i++)
   {
       ...
       for(int j = 0; j < dof ; j++)
       {
           int subscript = vdofs[vDim*dof + j]; // The dofs have first rho, then rho.u, rho.v, E
        ...
        int cycle, double time)
{
   ...
   GridFunction u1  (&fes_scalar);
   GridFunction u2  (&fes_scalar);
