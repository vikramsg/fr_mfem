...
void getAuxGrad(int dim, const HypreParMatrix &K_x, const HypreParMatrix &K_y, const CGSolver &M_solver, 
...
{
private:
   HypreParMatrix &M, &K_inv_x, &K_inv_y;
   ParLinearForm &b, &b_aux_x, &b_aux_y;
   ParGridFunction &u_sol, &f_inv, &aux_sol, &aux_grad, &f_vis;
   HypreSmoother M_prec;
   CGSolver M_solver;

public:
   FE_Evolution(HypreParMatrix &_M, HypreParMatrix &_K_inv_x, HypreParMatrix &_K_inv_y, 
                            ParLinearForm &_b, ParLinearForm &_b_aux_x, ParLinearForm &_b_aux_y,
...
private:
    ...
    ParBilinearForm *m, *k_inv_x, *k_inv_y;
...
//          new DG_Viscous_Aux_Integrator(y_dir, aux_vec, 1.0));
   ...
   adv  = new FE_Evolution(*M, *K_inv_x, *K_inv_y, 
           *b, *b_aux_x, *b_aux_y,
   ...
   if (viscous)
   {
        ...
        getAuxGrad(dim, *K_inv_x, *K_inv_y, adv->GetMSolver(), u_sol, *b_aux_x, *b_aux_y, aux_grad);
...
FE_Evolution::FE_Evolution(HypreParMatrix &_M, HypreParMatrix &_K_inv_x, HypreParMatrix &_K_inv_y, 
                            ParLinearForm &_b, ParLinearForm &_b_aux_x, ParLinearForm &_b_aux_y,
                            ParGridFunction &_u_sol, ParGridFunction &_f_inv, ParGridFunction &_aux_sol, 
                            ParGridFunction &_aux_grad, ParGridFunction &_f_vis)
   : TimeDependentOperator(_b.Size()), M(_M), K_inv_x(_K_inv_x), K_inv_y(_K_inv_y), 
                            b(_b), b_aux_x(_b_aux_x), b_aux_y(_b_aux_y),
...
void FE_Evolution::Mult(const Vector &x, Vector &y) const
{
    ...
    if (viscous)
    {
        ...
        getAuxGrad(dim, K_inv_x, K_inv_y, M_solver, x, b_aux_x, b_aux_y, aux_grad);
...
/* To check whether FaceNbrData can be used to get all required data
 * for linearform assemble
 */
void testCompute(ParFiniteElementSpace &pfes, const ParGridFunction &uD, 
        VectorGridFunctionCoefficient &u_coeff)
{
    int myid, num_procs;  
    MPI_Comm_rank(MPI_COMM_WORLD, &myid);
    ...
    if (num_procs > 1)
    {
        for (int i = 0; i < pmesh->GetNSharedFaces(); i++)
        {
            ...
            u2_dir.SetSize(var_dim);
            u2_v_dir.SetSize(var_dim);
            ...
            {
               order = tr->Elem1->OrderW() + 2*fe1->GetOrder();
            }
            ...
            for (int p = 0; p < ir->GetNPoints(); p++)
            {
               ...
               u_coeff.Eval(u2_dir, *tr->Elem2, eip2);
               ...
               for (int j = 0; j < var_dim; j++)
               {
                   ...
                   {
                       u2_v_dir[j] += shape2[k]*uD.FaceNbrData()[vdofs2[j*ndof2 + k]];
                   }
               }
...
//                   cout << u2_dir[0] << "\t" << u2_dir[1] << "\t" << uD.FaceNbrData()[vdofs2[1]] 
//                       << "\t" << uD.FaceNbrData()[vdofs2[ndof2 + 1]]<< endl;
                   cout << u2_dir[2] - u2_v_dir[2] << "\t" << u2_dir[3] - u2_v_dir[3] << endl; 
               }
            }
...
//                {
////                   cout << i << "\t" << j << "\t" << uD.Size() << "\t" << vdofs1[j] << "\t" << vdofs2[j] << endl;        
////                    cout << i << "\t" << j << "\t" << uD[vdofs1[j]] << "\t" << uD.FaceNbrData()[vdofs2[j]] << endl;        
//                }
//            }
        }
    }
    ...
    if (num_procs == 1)
    {
        for (int i = 0; i < pmesh->GetNumFaces(); i++)
        {
            ...
            fe1 = pfes.GetFE(tr->Elem1No);
            fe2 = pfes.GetFE(tr->Elem2No);
    
            pfes.GetElementVDofs(tr->Elem1No, vdofs1);
            pfes.GetElementVDofs(tr->Elem2No, vdofs2);
    
            cout << i << "\t" << vdofs1.Size() << "\t" << vdofs2.Size() << endl;
       
            for (int j = 0; j < vdofs1.Size(); j++)
            {
//                cout << i << "\t" << j << "\t" << uD.Size() << "\t" << vdofs1[j] << "\t" << vdofs2[j] << endl;        
                cout << i << "\t" << j << "\t" << uD[vdofs1[j]] << "\t" << uD[vdofs2[j]] << endl;        
            }
