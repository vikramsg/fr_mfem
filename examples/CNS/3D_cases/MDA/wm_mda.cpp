...
const int  restart_cycle     =1197000; // File number used for restart
   ...
   const FiniteElement &el, FaceElementTransformations &Trans, Vector &elvect)
{
   ...
   for (int p = 0; p < ir->GetNPoints(); p++)
   {
      ...
      getVectorKEPFlux(R, gamm, dim, u1_dir, u2_dir, nor, f_dir); // Get interaction flux at face using local Lax Friedrichs
   ...
   const FiniteElement &el, FaceElementTransformations &Tr, Vector &elvect)
{
   ...
   const double *pts = poly.GetPoints(elOrder, 1);

   double eps = 1E-14;

   
   // Get normal direction in reference plane
   // Once we get the normal direction the wall modeling point is just
   // the projection of the normal on the opposite face of the reference element
   // This is an approximation assuming the reference element is not too skewed
   Vector tan_ref(dim), nor_ref(dim);
   {
       // First point, fp + 1, last point on the face
       int fp = 0, fp1 = 1, lp = ir->GetNPoints() - 1;
       IntegrationPoint eipfp, eipfp1, eiplp;

       const IntegrationPoint &ip1 = ir->IntPoint(fp);
       Tr.Loc1.Transform(ip1, eipfp);
   
       Tr.Face->SetIntPoint(&ip1);
       Tr.Elem1->SetIntPoint(&eipfp);

       const IntegrationPoint &ip2 = ir->IntPoint(fp1);
       Tr.Loc1.Transform(ip2, eipfp1);
   
       Tr.Face->SetIntPoint(&ip2);
       Tr.Elem1->SetIntPoint(&eipfp1);

       const IntegrationPoint &ip3 = ir->IntPoint(lp);
       Tr.Loc1.Transform(ip3, eiplp);
   
       Tr.Face->SetIntPoint(&ip3);
       Tr.Elem1->SetIntPoint(&eiplp);
       ...
       if (dim == 2)
       {
           tan_ref[0] = eiplp.x - eipfp.x ;
           tan_ref[1] = eiplp.y - eipfp.y ;
       ...
       {
           Vector t1(dim), t2(dim);       
           
           t1[0] = eipfp1.x - eipfp.x ;
           t1[1] = eipfp1.y - eipfp.y ;
           t1[2] = eipfp1.z - eipfp.z ;

           t2[0] = eiplp.x  - eipfp.x ;
           t2[1] = eiplp.y  - eipfp.y ;
           t2[2] = eiplp.z  - eipfp.z ;

           // ASSUMPTION ALERT
           // This way of calculating normals seems to always point away from
           // a boundary element so we negate
           nor_ref[0] =  -t1[1]*t2[2] + t2[1]*t1[2];
           nor_ref[1] =   t1[0]*t2[2] - t2[0]*t1[2]; 
           nor_ref[2] =  -t1[0]*t2[1] + t2[0]*t1[1]; 
       }
       double ref_l2 = nor_ref.Norml2();
       nor_ref.Set(1./ref_l2, nor_ref); 
   }

   ElementTransformation *Elem1 = Tr.Elem1; // We need y co-ordinate to calculate y+
   Vector loc1(dim), loc2(dim);
   ...
   for (int p = 0; p < ir->GetNPoints(); p++)
   {
      ...
      u_bnd.Eval(u2_bnd, *Tr.Elem1, eip1);

      auxD.Eval(aux1_dir, *Tr.Elem1, eip1);

      double y_wm = 0.0;
      {
          eip2.x = eip1.x + nor_ref[0];
          eip2.y = eip1.y + nor_ref[1];
          if (dim == 3)
              eip2.z = eip1.z + nor_ref[2];

          Elem1->Transform(eip1, loc1);
          Elem1->Transform(eip2, loc2);
          
          for (int it = 0; it < dim; it++)
              y_wm += std::pow( loc2[it] - loc1[it], 2 );
          y_wm = std::sqrt(y_wm);
      }
      ...
      Vector vel_wm(dim);
      for(int it = 0; it < dim; it++) 
          vel_wm[it] = u_wm_dir[1 + it]/rho_wm;

      double v_wm_n = vel_wm*nor_dim;
      for(int it = 0; it < dim; it++) 
          vel_wm[it] = vel_wm[it] - v_wm_n*nor_dim[it]; 
      
      double v_wm_sq = 0.0;
      for(int it = 0; it < dim; it++) 
          v_wm_sq += std::pow( vel_wm[it], 2 ); 

      double u_wm    = std::sqrt(v_wm_sq);
      ...
      Vector fn_bnd(var_dim), vn_bnd(dim);
      fn_bnd = 0.0;
      for (int it = 0; it < dim; it++)
      {
          /*
           * We allocated boundary values to the tangential direction 
           * according to the ratio of velocity in each direction
           * The negative sign is because the normals point away from the walls
           * and tau_wall is calculated with normals pointing into the wall
           */
          fn_bnd[1 + it] = -tau_wm*vel_wm[it]*nor_l2/u_wm;
      }
      aux2_dir  = aux1_dir;
      double qn = 0.0; // Heat flux, just copy the derivatives
      for (int it = 0; it < dim; it++)
      {
          qn += (mu/Pr)*(gamm*R_gas/(gamm - 1))*aux2_dir(it*aux_dim + aux_dim - 1)*nor[it];
      }
      for (int it = 0; it < dim; it++)
      {
          vn_bnd[it] = u1_dir[1 + it]/u1_dir[0];
      }
      double vn_bd = vn_bnd*nor_dim, vtau = 0.0; // Get the force part of energy flux
      for (int it = 0; it < dim; it++)
      {
          vn_bnd[it] = vn_bnd[it] - vn_bd*nor_dim[it]; 
          vtau      += vn_bnd[it]*fn_bnd[1 + it];
      }
      fn_bnd[var_dim - 1] = qn + vtau; // Assuming all the signs are correct

      fD.Eval(f1_dir, *Tr.Elem1, eip1);        // Get discontinuous flux at face
      ...
      for (int i = 0; i < dim; i++)
      {
          for (int j = 0; j < var_dim; j++)
          {
              face_f(j) += tau*nor(i)*nor_dim(i)*(u2_dir(j) - u1_dir(j)); 
          }
      }
      ...
      if ( (Tr.Elem1No == 4) || (Tr.Elem1No == 288) )
      {
//          cout << Tr.Elem1No << "\t" << nor_dim[0] << "\t" << nor_dim[1] << endl; 
//          cout << loc1[0]    << "\t" << loc1[1]    << "\t" << loc1[2] << endl; 
//          cout << f_dir[1] << "\t" << f_dir[2] << "\t" << f_dir[3] << "\t" << f_dir[4] << endl;
//          cout << f_dir[6] << "\t" << f_dir[7] << "\t" << f_dir[8] << "\t" << f_dir[9] << endl;
//          cout << f_dir[11] << "\t" << f_dir[12] << "\t" << f_dir[13] << "\t" << f_dir[14] << endl;
//          cout << face_f[1] << "\t" << face_f[2] << "\t" << face_f[3] << "\t" << face_f[4] << endl;
//          cout << fn_bnd[1] << "\t" << fn_bnd[2] << "\t" << fn_bnd[3] << "\t" << fn_bnd[4] << endl;
      }
...
CNS::CNS() 
{
   ...
   int bdr_wall = 2; // Specify which marker signifies wall
   vector<double> x_uni, y_uni;
   GetUniqueX(*fes_temp, bdr_wall, x_uni, y_uni);
...
//   Bnd_Inv_NoSlip_Isotherm_Integrator *dg_ns = new Bnd_Inv_NoSlip_Isotherm_Integrator(
   Bnd_Euler_Slip_Isotherm_Integrator *dg_ns = new Bnd_Euler_Slip_Isotherm_Integrator(
              R_gas, gamm, u_vec, f_vec, u_wall_bnd, -1.0); 
   dg_ns->SetIntRule(&GLIntRules.Get(Geometry::SQUARE, 2*np-3));
   
   DG_Euler_Characteristic_Integrator *dg_char = new DG_Euler_Characteristic_Integrator(
              R_gas, gamm, u_vec, f_vec, u_char_bnd, -1.0); 
   dg_char->SetIntRule(&GLIntRules.Get(Geometry::SQUARE, 2*np-3));
...
//   DG_CNS_Vis_Isotherm_Integrator *dg_vis_iso = new DG_CNS_Vis_Isotherm_Integrator(
   ...
   b->AddBdrFaceIntegrator(dg_char, dir_bdr_char); 
