...
double ComputeGLTKE(ParGridFunction &uD);
...
CNS::CNS() 
{
   ...
   DGMassIntegrator *dgm = new DGMassIntegrator(1.) ;
   dgm->SetIntRule(&GLIntRules.Get(Geometry::SQUARE, 2*np-3));

   ParBilinearForm *m = new ParBilinearForm(fes);
   m->AddDomainIntegrator( dgm );
   m->Assemble(1); 
   m->Finalize(1); 
...
//       double a22      = 0.5  +     (.5/std::sqrt(3.)); 
       ...
       *u_sol = *k_s;
       ...
       double loc_tke, glob_tke;
       loc_tke = ComputeGLTKE(*u_sol);
       MPI_Allreduce(&loc_tke, &glob_tke, 1, MPI_DOUBLE, MPI_SUM, comm); 

       if (myid == 0)
       {
           flo_file << setprecision(12) << ti << "\t" << t << "\t" << glob_tke << endl;
       }
...
void CNS::newton_solve(const double a21, const double a22, const Vector &e_rhs0, Vector &e_rhs) 
{
    ...
    for (it = 0; it < max_newt_it; it++)
    {
        ...
        DGMassIntegrator *dgm = new DGMassIntegrator(1./(a22*dt_real)) ;
        dgm->SetIntRule(&GLIntRules.Get(Geometry::SQUARE, 2*np-3));
        DeriJacobianIntegrator *dj  = new DeriJacobianIntegrator(u_vec); 
        dj ->SetIntRule(&GLIntRules.Get(Geometry::SQUARE, 2*np-3));
        FaceJacobianIntegrator *fj = new FaceJacobianIntegrator(u_vec, *u_sol, *fes, glob_u_max);
        fj ->SetIntRule(&GLIntRules.Get(Geometry::SEGMENT, 2*np-3));

        // Matrix M/(a22*dt) + KJ
        // where K is the derivative operator and J the Jacobian
        ParBilinearForm *KJx = new ParBilinearForm(fes);
        KJx->AddDomainIntegrator( dgm );
        KJx->AddDomainIntegrator( dj );
        KJx->AddInteriorFaceIntegrator( fj );
        KJx->Assemble(1); 
        KJx->Finalize(1); 
...
double ComputeGLTKE(ParGridFunction &uD)
{
   FiniteElementSpace *fes = uD.FESpace();
