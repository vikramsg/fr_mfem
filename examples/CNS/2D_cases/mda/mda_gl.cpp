...
const double   mu  = 3.4895833E-7; 
const double R_gas = 287.;
const double   Pr  = 0.71;

//Run parameters
const char *mesh_file        =  "2d_mda_c2_dfq.msh";
...
const int  restart_freq      = 12000; // Create restart file after every 1000 time steps
const int  restart_cycle     = 90000; // File number used for restart
...
CNS::CNS() 
{
   ...
   *f_I_b = *f_inv;
   ...
   DG_Euler_NoSlip_Integrator *dg_ns = new DG_Euler_NoSlip_Integrator(
   ...
   DG_CNS_Vis_Adiabatic_Integrator *dg_vis_adi = new DG_CNS_Vis_Adiabatic_Integrator(
               R_gas, gamm, u_vec, vis_vec, aux_grad_vec, u_adi_wall_bnd, mu, Pr, 1.0);
   dg_vis_adi->SetIntRule(&GLIntRules.Get(Geometry::SQUARE, 2*np-3));

   b->AddBdrFaceIntegrator(dg_vis_adi, dir_bdr_wall);
