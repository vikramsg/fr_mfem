...
double getMaxDivFlux(int dim, const Vector &u, const Vector &aux_grad);
...
void getKGSplitDx(int dim, 
        const HypreParMatrix &K_x, const HypreParMatrix &K_y, const HypreParMatrix &K_z, 
        const Vector &u, Vector &f_dx); 
void getChandrashekharSplitDx(int dim, 
        const HypreParMatrix &K_x, const HypreParMatrix &K_y, const HypreParMatrix &K_z, 
        const Vector &u, Vector &f_dx); 
...
CNS::CNS() 
{
   ...
   const IntegrationRule *ir_vol, *ir_sur;
   if (dim == 2)
   {
       ir_vol = &GLIntRules.Get(Geometry::SQUARE , 2*np-3);
       ir_sur = &GLIntRules.Get(Geometry::SEGMENT, 2*np-3);
   }
   else if (dim == 3)
   {
       ir_vol = &GLIntRules.Get(Geometry::CUBE  , 2*np-3);
       ir_sur = &GLIntRules.Get(Geometry::SQUARE, 2*np-3);
   }

   MassIntegrator *mass_integ = new MassIntegrator;
   mass_integ->SetIntRule(ir_vol);

   ConvectionIntegrator cix(x_dir, -1.0); 
   cix.SetIntRule(ir_vol);
   ConvectionIntegrator ciy(y_dir, -1.0);
   ciy.SetIntRule(ir_vol);
   ...
   delete m, k_inv_x, k_inv_y, k_vis_x, k_vis_y;
   ...
   *f_I_b = *f_inv;
   ...
   for (ti = ti_in; !done; )
   {
      ...
      double eta = ComputeGLEntropy(*fes, *u_sol);
      double glob_tk, glob_eta;
      MPI_Allreduce(&tk,   &glob_tk,   1, MPI_DOUBLE, MPI_SUM, comm); // Get global across processors
      MPI_Allreduce(&eta,  &glob_eta,  1, MPI_DOUBLE, MPI_SUM, comm); // Get global across processors

      if (myid == 0)
      {
          flo_file << setprecision(12) << ti << "\t" << t << "\t" << glob_tk << "\t" << glob_eta << endl;
      }
...
void FE_Evolution::Mult(const ParGridFunction &x, ParGridFunction &y) const
{
    ...
    double max_div = getMaxDivFlux(dim, x, u_grad);
...
//    cout << "Max div " << max_div << endl;
    ...
    if (split == true)
    {
        ...
        else if ( splitform == "Chandra" ) 
            getChandrashekharSplitDx(dim, K_inv_x, K_inv_y, K_inv_z, x, f_dx);
        else if ( splitform == "Lax" )
            getLaxSplitDx(dim, K_inv_x, K_inv_y, K_inv_z, x, f_dx);
        else
...
double getMaxDivFlux(int dim, const Vector &u, const Vector &aux_grad)
{
    ...
    for(int i = 0; i < offset; i++)
    {
        ...
        if (i == 0)
            max_div = std::abs(divergence);
        else
            max_div = std::max(max_div, std::abs(divergence));
    }
        ...
        const Vector &u, Vector &f_dx) 
{
    ...
    double lambda = getUMax(dim, u);
    
    Vector f_I(dim*var_dim*offset);
    getInvFlux(dim, u, f_I); 
    
    Vector f_sol(offset), f_x(offset);
    Vector u_sol(offset), u_x(offset);
    
    Vector temp1(offset), temp2(offset), temp(offset);

    Vector y(var_dim*offset), y_temp(var_dim*offset);
    y = 0.0;
    ...
    for(int i = 0; i < var_dim; i++)
    {
        ...
        getSparseMat(u_sol).Mult(u_sol,   temp);
        K_x.Mult(temp, temp1);
        temp1 *= 0.5;
        
        getSparseMat(u_sol).Mult(u_x,    temp2);
        temp2 *= -1;
...
//            u.GetSubVector(offsets[i - var_dim], u_sol);
//            K_z.Mult(u_sol, u_x);
//
//            u_x *= -lambda;
//        
...
void get_Bug_KGSplitDx(int dim, 
        const HypreParMatrix &K_x, const HypreParMatrix &K_y, const HypreParMatrix &K_z, 
        const Vector &u, Vector &f_dx) 
{
    int var_dim = dim + 2;
        ...
        const Vector &u, Vector &f_dx) 
{
    ...
    K_x.Mult(T ,          T_dx );
    K_x.Mult(rho_T ,  rho_T_dx );
    K_x.Mult(T_sq  ,  T_sq_dx  );
    K_x.Mult(rho_T_sq, rho_T_sq_dx);
    ...
    
    {
        ...
        rho_mat.              Mult(T_dx     ,     temp1);
        add(temp1, rho_T_dx, temp2);
        temp2 *= 3*0.5*R_gas;

        temp += temp2;

        rho_mat.              Mult(invT               ,     temp1);
        getSparseMat(temp1)  .Mult(T_sq_dx            ,     temp2);
        temp2 *= 0.5*R_gas;
        
        getSparseMat(invT)   .Mult(rho_T_sq_dx,     temp1);
        temp1 *= 0.5*R_gas;
...
//        temp  += pres_dx;
    ...
    K_y.Mult(T ,          T_dx );
    K_y.Mult(rho_T ,  rho_T_dx );
    K_y.Mult(T_sq  ,  T_sq_dx  );
    K_y.Mult(rho_T_sq, rho_T_sq_dx);
...
double ComputeGLTKE(ParFiniteElementSpace &fes, const Vector &uD)
{
   ...
   for (int i = 0; i < fes.GetNE(); i++)
   {
       ...
       if (dim == 2)
       {
           ir = &GLIntRules.Get(Geometry::SEGMENT, 2*np-3);
       }
       else if (dim == 3)
       {
           ir = &GLIntRules.Get(Geometry::SQUARE, 2*np-3);
...
double ComputeGLEntropy(ParFiniteElementSpace &fes, const Vector &uD)
{
   ...
   for (int i = 0; i < fes.GetNE(); i++)
   {
       ...
       for (int p = 0; p < ir->GetNPoints(); p++)
       {
           ...
           double vel[dim];        
           for(int j = 0; j < dim; j++) vel[j]   = uD[vdofs[(1 + j)*dof + p]]/rho;

           double vel_sq = 0.0;
           for(int j = 0; j < dim; j++) vel_sq += pow(vel[j], 2);

           double E       = uD[vdofs[(var_dim - 1)*dof + p]];
           double pres    = (E - 0.5*rho*vel_sq)*(gamm - 1);
      
           double point_eta = 0.0;       

           double  s  = log(pres) - gamm*log(rho);
           point_eta +=  -rho*s/(gamm - 1);

           eta += ip.weight*T->Weight()*point_eta;
       }
   }
