...
const int  restart_cycle     =  5000; // File number used for restart
...
CNS::CNS() 
{
   ...
   *f_I_b = *f_inv;
   ...
   for (ti = ti_in; !done; )
   {
      ...
      if (myid == 0)
      {
          flo_file << setprecision(14) << ti << "\t" << t << "\t" << glob_tk << "\t" << glob_eta
                   << "\t" << glob_rho_min 
                   << "\t" << glob_l2error << endl;
      }
...
void GetL2Error(ParGridFunction &u, const double &time, double &l2error)
{
    ...
    for (int i = 0; i < fes.GetNE(); i++)
    {
        ...
        for (int p = 0; p < ir->GetNPoints(); p++)
        {
            ...
            l1error += ip.weight*T->Weight()*std::abs( pres );

        }
    
    }
//    cout << u.Sum() << "\t" << l1error << "\t" << l2error << endl;
