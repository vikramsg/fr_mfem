...
const int    order           =  3;
const double t_final         =  20.5001 ;
const int    problem         =  0;
const int    ref_levels      =  5;

const bool   time_adapt      =  true ;
const double cfl             =  0.1  ;
const double dt_const        =  0.01  ;
...
void GetL2Error(ParGridFunction &u, const double &time, double &l2error);
...
CNS::CNS() 
{
   ...
   *f_I_b = *f_inv;
   ...
   for (ti = ti_in; !done; )
   {
      ...
      double l2error, glob_l2error; 
      GetL2Error(*u_sol, t, l2error);
      MPI_Allreduce(&l2error, &glob_l2error, 1, MPI_DOUBLE, MPI_SUM, comm); // Get global across processors

      glob_l2error = glob_l2error/100.; 

      if (myid == 0)
      {
          flo_file << setprecision(14) << ti << "\t" << t << "\t" << glob_tk << "\t" << glob_ten
                   << "\t" << glob_rho_min 
                   << "\t" << glob_l2error << endl;
      }
                                          ...
                                          Vector &b_nl) const
{
   ...
   if (riemann == "LF")
       ...
       getVectorSF3Flux(R_gas, gamm, dim, u_l, u_r, nor_face, f_com);
   else if (riemann == "Roe")
...
void FE_Evolution::getKGRestriction(const ParGridFunction &u, Vector &f_l, Vector &f_r) const
{
...

   {
       ...
       f_l.SetSubVector(offsets_face[dim + 1], temp);
...

   {
       ...
       f_l.SetSubVector(offsets_face[var_dim + dim + 1], temp);
...

   {
       ...
       f_r.SetSubVector(offsets_face[dim + 1], temp);
...

   {
       ...
       f_r.SetSubVector(offsets_face[var_dim + dim + 1], temp);
...
void FE_Evolution::getKGCorrection(const ParGridFunction &u, Vector &f_ke_corr) const
{
   ...
   for (int j = 0; j < n_face_pts; j++)
   {
       ...
       if (dim == 3)
           ke_face_error_l[j] += 0.5*( rho_vel_Rl[0][j]*vel_sq_Rl[2][j] 
                                                            - rho_vel_Rl[0][j]*vel_Rl[2][j]*vel_Rl[2][j] );
       ...
       if (dim == 3)
           ke_face_error_l[j] += 0.5*( rho_vel_Rl[1][j]*vel_sq_Rl[2][j] 
                                                            - rho_vel_Rl[1][j]*vel_Rl[2][j]*vel_Rl[2][j] );
       ...
       if (dim == 3)
       {
           ...
           ke_face_error_l[j] += 0.5*( rho_vel_Rl[2][j]*vel_sq_Rl[0][j] 
                                                            - rho_vel_Rl[2][j]*vel_Rl[0][j]*vel_Rl[0][j] );
           ke_face_error_l[j] += 0.5*( rho_vel_Rl[2][j]*vel_sq_Rl[1][j] 
                                                            - rho_vel_Rl[2][j]*vel_Rl[1][j]*vel_Rl[1][j] );
           ke_face_error_l[j] += 0.5*( rho_vel_Rl[2][j]*vel_sq_Rl[2][j] 
                                                                - rho_vel_Rl[2][j]*vel_Rl[2][j]*vel_Rl[2][j] );
   ...
   for (int j = 0; j < n_face_pts; j++)
   {
       ...
       if (dim == 3)
           ke_face_error_r[j] += 0.5*( rho_vel_Rr[0][j]*vel_sq_Rr[2][j] 
                                                                - rho_vel_Rr[0][j]*vel_Rr[2][j]*vel_Rr[2][j] );
       ...
       if (dim == 3)
           ke_face_error_r[j] += 0.5*( rho_vel_Rr[1][j]*vel_sq_Rr[2][j] 
                                                    - rho_vel_Rr[1][j]*vel_Rr[2][j]*vel_Rr[2][j] );
       ...
       if (dim == 3)
       {
           ...
           ke_face_error_r[j] += 0.5*( rho_vel_Rr[2][j]*vel_sq_Rr[0][j] 
                                                            - rho_vel_Rr[2][j]*vel_Rr[0][j]*vel_Rr[0][j] );
           ke_face_error_r[j] += 0.5*( rho_vel_Rr[2][j]*vel_sq_Rr[1][j] 
                                                            - rho_vel_Rr[2][j]*vel_Rr[1][j]*vel_Rr[1][j] );
           ke_face_error_r[j] += 0.5*( rho_vel_Rr[2][j]*vel_sq_Rr[2][j] 
                                                            - rho_vel_Rr[2][j]*vel_Rr[2][j]*vel_Rr[2][j] );
        ...
        const Vector &u, Vector &f_dx) 
{
  ...
  
    {
        ...
        fx.SetSubVector(offsets[dim + 1], temp);
...

    {
        ...
        fx.AddElementVector(offsets[dim + 1], temp);
    }
...
void GetL2Error(ParGridFunction &u, const double &time, double &l2error)
{
    ...
    for (int i = 0; i < fes.GetNE(); i++)
    {
        ...
        ord   = 2*el->GetOrder() + 1;
        ir    = &IntRules.Get(el->GetGeomType(), ord  );
        ...
        for (int p = 0; p < ir->GetNPoints(); p++)
        {
            ...
            double loc_rho    = u[vdofs[p]];

            int per_dis = std::abs( int( (loc[0] - xc[0] )/5. ) );
            double per_exp = -1*std::pow(1, per_dis);
            if (per_dis == 0)
                per_exp = 0; 
            double rx =  -(loc[0] - xc[0]) + 10.0*per_exp ;

            r      = std::sqrt( (std::pow( rx , 2) + std::pow(loc[1] - xc[1], 2)) );
            omega1 = std::exp(1 - r*r);
            omega2 = std::exp( 0.5*(1 - r*r) );
         
            rho    = std::pow( ( 1 - std::pow(beta, 2)*(gamm - 1)*omega1/
                        (8.*gamm*pow(M_PI, 2)) ), (1./(gamm - 1)) );
            u1     = M_inf - beta*loc[1]*omega2/(2.*M_PI);
            u2     =         beta*loc[0]*omega2/(2.*M_PI);
            
            pres   = std::pow(rho, gamm);

            l2error += ip.weight*T->Weight()*std::abs( rho - loc_rho );
