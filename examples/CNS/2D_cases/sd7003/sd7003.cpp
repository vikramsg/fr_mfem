...
private:
   ...
   LinearForm &b, &b_aux_x, &b_aux_y, &b_aux_z;
...
public:
   FE_Evolution(SparseMatrix &_M, SparseMatrix &_K_inv_x, SparseMatrix &_K_inv_y, SparseMatrix &_K_inv_z, 
                            ...
                            LinearForm &_b_aux_x, LinearForm &_b_aux_y, LinearForm &_b_aux_z, 
                            LinearForm &_b);
...
private:
    ...
    GridFunction *u_sol, *f_inv, *f_vis;   
    GridFunction *aux_sol, *aux_grad;   
    GridFunction *u_b, *f_I_b;   // Used in calculating b
    LinearForm   *b, *b_aux_x, *b_aux_y, *b_aux_z;

    ODESolver *ode_solver; 
    FE_Evolution *adv;
    GridFunction *u_t, *k_t, *y_t;   
...
CNS::CNS() 
{
   ...
   fes = new FiniteElementSpace(mesh, &fec, var_dim);

   int vSize = fes->GetTrueVSize();
   cout << "Number of unknowns: " << vSize << endl;

   VectorFunctionCoefficient u0(var_dim, init_function);
   
   u_sol = new GridFunction(fes);
   u_sol->ProjectCoefficient(u0);

   fes_vec = new FiniteElementSpace(mesh, &fec, dim*var_dim);
       
   f_inv = new GridFunction(fes_vec);
   getInvFlux(dim, *u_sol, *f_inv);

   f_vis = new GridFunction(fes_vec);

   FiniteElementSpace fes_aux(mesh, &fec, aux_dim);
   aux_sol = new GridFunction(&fes_aux);

   FiniteElementSpace fes_aux_grad(mesh, &fec, dim*aux_dim);
   aux_grad = new GridFunction(&fes_aux_grad);

   // For time stepping
   u_t = new GridFunction(fes);
   k_t = new GridFunction(fes);
   y_t = new GridFunction(fes);
   ///////////////////////////////////////////////////////////
   ...
   fes_op = new FiniteElementSpace(mesh, &fec);
   ...
   u_b    = new GridFunction(fes);
   f_I_b  = new GridFunction(fes_vec);
   ...
   *f_I_b = *f_inv;
   ...
   Array<int> dir_bdr_char(mesh->bdr_attributes.Max()); 
   dir_bdr_char = 0; dir_bdr_char[1] = 1; 
   Array<int> dir_bdr_wall(mesh->bdr_attributes.Max());
   ...
   b_aux_x = new LinearForm(&fes_aux);
   b_aux_y = new LinearForm(&fes_aux);
   ...
   if (dim == 3)
   {
       ...
       b_aux_z = new LinearForm(&fes_aux);
   ...
   SparseMatrix *K_inv_z;
   SparseMatrix *K_vis_z;

   ///////////////////////////////////////////////////////////////
   //Setup time stepping objects and do initial post-processing
   if (dim == 3)
   {
       *K_inv_z = k_inv_z->SpMat();
       *K_vis_z = k_vis_z->SpMat();
   ...
   {
       double u_max = getUMax(dim, *u_sol); // Get u_max on local processor
   ...
   for (ti = 0; !done; )
   {
      ...
      ComputeLift(*mesh, *fes, *u_sol, *f_vis, dir_bdr_wall, gamm, forces);
...
void CNS::Step()
{
    ...
    *u_t = *u_sol;
    ...
    cout << "time step: " << ti << ", dt: " << dt_real << ", time: " << 
            t << ", max_speed " << u_max << ", fes_size " << fes->GetTrueVSize() << endl;
